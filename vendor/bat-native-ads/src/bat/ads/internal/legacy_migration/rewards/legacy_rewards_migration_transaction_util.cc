...
TransactionList GetUnreconciledTransactions(
    const TransactionList& transactions,
    const privacy::UnblindedPaymentTokenList& unblinded_payment_tokens) {
...
BuildTransactionsForReconciledTransactionsThisMonth(
  ...
  // Add |transaction_count| transactions with a value of 0.0 to migrate ads
  // received this month
  ...
  for (auto& transaction : reconciled_transactions) {
    transaction.reconciled_at = transaction.created_at;
  }
