...
bool Conversions::FromJson(const std::string& json) {
  ...
  DCHECK(conversion_id_patterns_value->is_dict());

  for (const auto kv : conversion_id_patterns_value->DictItems()) {
    if (!kv.second.is_dict()) {
      BLOG(1, "Failed to load from JSON, conversion pattern not of type dict")
      return false;
    }

    const std::string* id_pattern = kv.second.FindStringKey("id_pattern");
    if (!id_pattern || id_pattern->empty()) {
      BLOG(1, "Failed to load from JSON, pattern id_pattern missing");
      return false;
    }

    const std::string* search_in = kv.second.FindStringKey("search_in");
