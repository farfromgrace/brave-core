...
// 4. Program IDs will be put at last, unless there are accounts listed in
// instructions with the same pubkey.
void SolanaMessage::GetUniqueAccountMetas(
  ...
  // Get program ID from each instruction.
  for (const auto& instruction : instructions_) {
  ...
  std::sort(account_metas.begin(), account_metas.end(),
